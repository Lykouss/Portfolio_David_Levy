  ...
  return (
    <div className="flex h-full w-full flex-col bg-background">
      ...
      <div className="flex-shrink-0">
        {isAdmin ? (
            <div className="p-4 border-b border-secondary flex items-center gap-4">
              <button onClick={onBack} className="md:hidden text-text-muted hover:text-accent"><ArrowLeft /></button>
              <div>
                ...
                <div className="flex items-center gap-2 text-xs text-text-muted h-4">
                  <AnimatePresence mode="wait">
                    <motion.div key={isTyping ? 'typing' : (presenceStatus.isOnline ? 'online' : (presenceStatus.lastSeen || 'offline'))} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.2 }} className="flex items-center gap-2">
            ...
            <div className="p-4 border-b border-secondary flex justify-between items-center">
              ...
              <div className="flex items-center gap-6">
                <Link href="/" title="Voltar ao Portfólio" className="text-text-muted hover:text-accent transition-colors"><Home size={22} /></Link>
                <button onClick={handleLogout} title="Sair da conta" className="text-text-muted hover:text-accent transition-colors"><LogOut size={22} /></button>
              </div>
            </div>
        )}
      </div>
      ...
      <div className="flex-grow overflow-y-auto p-4">
        {loading ? (
            <div className="flex justify-center items-center h-full text-text-muted"><Loader2 className="animate-spin" size={24} /></div>
        ) : (
          <AnimatePresence>
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id}>
                  {dateSeparator}
                  <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className={`flex mb-2 ${isSentByMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs md:max-w-md py-2 px-3 rounded-lg flex items-end gap-2 ${isSentByMe ? 'bg-accent text-background' : 'bg-primary'}`}>
                      ...
                      <div className="flex-shrink-0 text-xs opacity-70 flex items-center" style={{ color: isSentByMe ? 'rgba(255,255,255,0.7)' : '' }}>
                        ...
                        {isSentByMe && (<motion.div layout className="ml-1">{msg.isRead ? <CheckCheck size={16} className="text-blue-400"/> : <Check size={16} />}</motion.div>)}
                      </div>
                    </div>
                  </motion.div>
                </div>
      ...
      <div className="flex-shrink-0 p-4 border-t border-secondary">
        <form onSubmit={handleSendMessage} className="flex gap-4">
          <input type="text" value={newMessage} onChange={handleTyping} placeholder="Digite sua mensagem..." className="flex-grow p-3 bg-primary border border-secondary rounded-lg focus:outline-none focus:ring-2 focus:ring-accent transition-all" />
          <motion.button type="submit" className="bg-accent text-background p-3 rounded-lg hover:bg-accent-hover transition-colors disabled:opacity-50" disabled={!newMessage.trim()} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><Send /></motion.button>
        </form>
      </div>
    </div>
  );
